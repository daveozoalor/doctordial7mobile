...
var messageList = $$('.messageCountBadge');
...
var count = 0;
    ...
    //find a message log where this user is the sender and update it
			 messageslogRef.orderByChild('seen').equalTo(0).on("child_added", function(snapshot) {
			 count++;  
			  
              if(count > 0){
              	//alert("Seen is greater than 0");
              	$$('.messageCount').remove();
               messageList.append('<span class="badge bg-red messageCount">'+count+'</span>');   
              }
           });
}
